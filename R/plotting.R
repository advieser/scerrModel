...
#' the percentage of remaining faults per round, and the observed effect sizes per round.
#'
...
#'  If `TRUE`, the cost-benefit-ratios will be plotted additionally in the first plot. Default is `FALSE`.
#' @param true_effect (`logical()`)\cr
#'  If `TRUE`, the true effect sizes of each study will be plotted additionally in the third plot. Default is `FALSE`.
#'
#' @return
#' A `patchwork` / `gg` object that can be further modified using the `patchwork` package.
#'
#' @examples
...
#'
#' @export
plot_summary_panel <- function(literature, study_id = NULL, cbr = FALSE, true_effect = FALSE) {
  ...
  possible_study_ids <- setdiff(names(literature), c("complete_studies", "seed", "use_same_seed"))
  ...
  gg1 <- plot_prob_fault_this_round(literature, study_id, cbr = cbr)
  gg2 <- plot_undiscovered_faults(literature, study_id) +
    ggplot2::guides(color = "none")
  gg3 <- plot_obs_effect_sizes(literature, study_id, true_effect = true_effect) +
    ggplot2::guides(color = "none")
...
#' Plot Percentage of Undiscovered Faults per Round
#'
#' This plots the percentage of undiscovered faults per round, i.e. the number of faults remaining divided by the number
#' of code units remaining.
#'
#' @param literature (`list()`)\cr
#' A list containing the simulated literature, generated by [simulate_literature()].
#' @param study_id (`character()` or `integer()`)\cr
...
#' plot_undiscovered_faults(lit)
#' }
#'
#' @export
plot_undiscovered_faults <- function(literature, study_id = NULL) {
  # Check if ggplot2 is available
  if (!requireNamespace("ggplot2", quietly = TRUE)) {
    stop("The 'ggplot2' package is needed for this function to work. Please install it.", call. = FALSE)
  }
  ...
  # Extract vectors of remaining faults per round (relative to remaining code units)
  perc_rem_faults <- lapply(studies, function(x) {
    rev(cumsum(x[["fault_indicators"]])) / seq(length(x[["fault_indicators"]]), 1)
  })
  # Pad vectors to assure same length
  perc_rem_faults <- lapply(perc_rem_faults, function(x) {
    length(x) <- max(lengths(perc_rem_faults))
    x
  })

  df <- data.frame(
    study_id = rep(names(perc_rem_faults), each = length(perc_rem_faults[[1]])),
    round = rep(seq_along(perc_rem_faults[[1]]), times = length(perc_rem_faults)),
    perc_rem_faults = unname(unlist(perc_rem_faults))
  )

  # Image generation
  gg <- ggplot2::ggplot(df, ggplot2::aes(y = perc_rem_faults, x = round, color = study_id)) +
    ggplot2::geom_line() +
    ggplot2::labs(title = "Percentage of Undiscovered Faults per Round",
                  x = "Round", y = "Undiscovered Faults in %") +
    ggplot2::theme_minimal()
...
#' A vector of (possibly multiple) unique identifiers of studies indicating for which studies the plot should be generated.
#' If `NULL`, all studies in `literature` are plotted. Default is `NULL`.
#' @param binwidth (`numeric()`)\cr
#' The width of the bins in the histogram. Default is `0.3`.
#'
#' @return
#' A `ggplot` object that can be further customized using the `ggplot2` package.
#'
...
#'
#' @export
plot_histogram_final_effect_sizes <- function(literature, study_id = NULL, binwidth = 0.3) {
  # Check if ggplot2 is available
  if (!requireNamespace("ggplot2", quietly = TRUE)) {
    stop("The 'ggplot2' package is needed for this function to work. Please install it.", call. = FALSE)
  }
  ...
  # Image generation
  gg <- ggplot2::ggplot(df, ggplot2::aes(x = final_obsereved_effect_sizes)) +
...
#' Default is `"relative"`.
#' @param binwidth (`numeric()`)\cr
#' The width of the bins in the histogram. If `NULL` and `type = "relative"`, the binwidth is set to `0.1`. If `NULL` and
#' `type = "absolute"`, the binwidth is set to `1`. Default is `NULL`.
#'
#' @return
#' A `ggplot` object that can be further customized using the `ggplot2` package.
#'
...
#'
#' @export
plot_histogram_n_faults <- function(literature, study_id = NULL, type = "relative", binwidth = NULL) {
  # Check if ggplot2 is available
  if (!requireNamespace("ggplot2", quietly = TRUE)) {
    stop("The 'ggplot2' package is needed for this function to work. Please install it.", call. = FALSE)
  }
  ...
  if (is.null(binwidth)) {
    binwidth <- if (type == "relative") 0.1 else 1
  }
  assert(
    check_character(study_id, any.missing = FALSE),
    check_integerish(study_id, lower = 1, any.missing = FALSE),
  ...
  if (type == "relative") {
    ...
    # Handle edge case where total number of faults is 0 in at least one study
    if (any(n_faults_total == 0)) {
      n_faults_total <- n_faults_total[!n_faults_total == 0]
      n_faults_undiscovered <- n_faults_undiscovered[names(n_faults_total)]
      warning("The total number of faults in at least one study is 0. These studies will not be plotted, if type = \"relative\".")
    }
    df = data.frame(
  ...
  # Image generation
  gg <- ggplot2::ggplot(df, ggplot2::aes(x = n_faults_undiscovered)) +
    ggplot2::geom_histogram(binwidth = binwidth, color = "black", fill = "gray") +
    ggplot2::labs(title = paste("Histogram of", type, "Number of Undiscovered Faults"),
...
plot_distributions_heatmap <- function(literature, study_id, type) {
  ...
  assert_subset(study_id, setdiff(names(literature), c("complete_studies", "seed", "use_same_seed")))
  assert_string(type)
