...
#'
#' @return A `list` of `ggplot` objects, one per study.
#'
#' @examples
...
#'
#' @export
plot_observed_effect_sizes <- function(literature) {
  require_package("ggplot2")
  require_package("ggtext")
  assert_literature(literature)
  ...
  observed_effect_sizes <- lapply(literature, function(study) {
    ...
    observed_effect_sizes <- true_effect + sum(error_sizes) - c(0, cumsum(error_sizes))
    data.frame(
      round = seq.int(0L, length(error_sizes)),
...
#' @inheritParams plot_posteriors
#'
#' @return A `list` of `patchwork` objects, one per study.
#'
#' @export
plot_study_panels <- function(literature) {
  ...
  effect_size_plots <- plot_observed_effect_sizes(literature)
